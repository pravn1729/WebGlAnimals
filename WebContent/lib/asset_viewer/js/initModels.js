...
//  //CUSTOM
  ...
  function callback(count, total){
	  if(counti == total)
	  switchModel(0);
  }
  
  for(var i=0;i<6;i++){
      modelLoader(i, 6, callback);
  }

}

function modelLoader(id, total, callback) {
...
function switchModel(id) {
  ...
  if (models[id].type == "animal"){
	  for(var i=0;i<6;i++){
		  morphObject = new ROME.Animal(models[i].geometry, true);
		morphObject.mesh.updateMatrix();
		morphObject.mesh.update();
		morphObject.mesh.position.x += i*100;
